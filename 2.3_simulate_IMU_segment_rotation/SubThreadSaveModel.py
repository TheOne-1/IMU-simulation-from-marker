...
def train_models(base_model, input_names, output_names, i_sub, folder_name):
    ...
    file_path = RESULT_PATH + folder_name + '\\model_' + date + '_subject_' + str(i_sub) + '.pkl'
...
def write_specification_file(model, input_names, output_names, folder_name):
    date = time.strftime('%Y%m%d')
    file_path = RESULT_PATH + folder_name + '\\model_' + date + '.txt'
    specification_txt_file = RESULT_PATH + folder_name + '\\' + date + '_specification.pkl'
    i_file = 0
    while os.path.isfile(file_path):
        i_file += 1
        file_path = RESULT_PATH + folder_name + '\\model_' + date + '_' + str(i_file) + '.txt'
        specification_txt_file = RESULT_PATH + folder_name + '\\model_' + date + '_' + str(i_file) + '_specification.txt'
    # with open(file_path, 'wb') as fp:
    #     pickle.dump(evaluator_dict, fp, protocol=4)
...
def get_segment_rotation_result(input_names, output_names, result_column, i_sub, folder_name, date):
    start_time = datetime.now()
    print('Subject ' + str(i_sub) + ', start at ' + start_time.strftime('%H:%M:%S'))
    ...
    for segment_moved in SEGMENT_NAMES:
        ...
        for speed in SPEEDS:
            ...
            for i_rotation in range(rotation_len):
                ...
                R2[i_rotation, :], RMSEs[i_rotation, :], NRMSEs[i_rotation, :] = my_evaluator.get_all_scores(y_test, y_pred)
...
def get_all_rotation_result(input_names, output_names, result_column, i_sub, folder_name, date, error_value=25):
    start_time = datetime.now()
    print('Subject ' + str(i_sub) + ', start at ' + start_time.strftime('%H:%M:%S'))
